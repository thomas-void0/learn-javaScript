...
Array.prototype.splice = function (start, deleteCount) {
    var argumentsLen = arguments.length; //得到传入的参数长度
    var array = Object(this); //得到调用splice方法的数组
    var len = array.length; //得到调用splice方法的数组的长度
    var deleteArr = new Array(deleteCount); //创建一个存储删除元素的数组
    var addElements = Array.prototype.slice.call(arguments, 2); //获取到需要添加的元素
    // 拷贝删除的元素
    sliceDeleteElements(array, start, deleteCount, deleteArr);
    //对元素进行位移操作
    if (addElements.length > 0 && deleteCount) {
        movePostElements(array, start, len, deleteCount, addElements);
    }
    // 对源数组进行替换操作
...
function sliceDeleteElements(array, start, deleteCount, deleteArr) {
    if (deleteCount) {
        ...
        for (var i = 0; i < array.length - start; i++) {
            var index = start + i;
            if (index in array) {
...
// 对数组元素进行位移操作
function movePostElements(array, start, len, deleteCount, addElements) {
    var addLen = addElements.length;
    ...
    if (deleteCount > addLen) {
        ...
        for (var i = start + deleteCount; i < len; i++) {
            ...
            var fromIndex = i;
            // 要被移动到的新位置的索引值
            var toIndex = i - (deleteCount - addLen);
            // 将原本位置上的元素进行覆盖
        ...
        // 把超过这个长度的数组元素都去除掉
        for (var i = len - 1; i >= len + addLen - deleteCount; i--) {
            delete array[i];
        }
    }
    // 3，删除数量小于替换数量
    if (deleteCount < addLen) {
        ...
        for (var i = len - 1; i >= start + deleteCount; i--) {
            ...
            var fromIndex = i;
            // 需要被挪动到的位置，这里是倒序的。
            var toIndex = i + (addLen - deleteCount);
            if (fromIndex in array) {
                array[toIndex] = array[fromIndex];
            }
            else {
...
// 边界情况处理
//对start索引值的处理
function computeStartIndex(start, len) {
    // 处理索引负数的情况,从反方向取索引值
    if (start < 0) {
